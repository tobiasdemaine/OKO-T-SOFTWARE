...
void loadconf (void){
	...
	FILE *filein;
	char oneline[255];
	filein = fopen("config.okot", "r");				// File To Load World Data From

	for (int loop = 0; loop < 27; loop++){
		readstr(filein,oneline);
		sscanf(oneline, "width= %d\n", &screen_width);
		sscanf(oneline, "height= %d\n", &screen_height);
...
{																		// TRUE If Successful
	...
	windowClass.hIcon			= LoadIcon(application->hInstance, "GENGL.ICO");
...
int WINAPI WinMain (HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	// Window Application
	window.init.title			= "OKO T";		        	// Window Title
	window.init.width			= screen_width;									// Window Width
	window.init.height			= screen_height;									// Window Height
	window.init.bitsPerPixel	= 16;									// Bits Per Pixel
	window.init.isFullScreen	= windowed;									// Fullscreen? (Set To TRUE)

	ZeroMemory (&keys, sizeof (Keys));									// Zero keys Structure

	// Ask The User If They Want To Start In FullScreen Mode?
//	if (MessageBox (HWND_DESKTOP, "Fullscreen Mode? F1 will toggle screen mode", "Start FullScreen?", MB_YESNO | MB_ICONQUESTION) == IDNO)
//	{
